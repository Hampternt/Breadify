    ...
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            ui.vertical(|ui| {
                ...
                ui.label(
                    RichText::new("100 % is one slot. A roll might be 50, something bulky 200.")
                        .family(theme::mono())
                        .size(11.0)
                        .color(theme::FAINT),
                );
...
fn sizes(app: &mut Breadify, ui: &mut egui::Ui) {
    ...
    for (id, name) in products {
        let mut percent = app.settings.crates.size_of(id);
        ui.horizontal(|ui| {
            let response = ui.add(
            ...
            ui.label(RichText::new(name).family(theme::body()).size(12.0).color(
            ...
            if response.changed() {
                changed = Some((id, percent));
            }
        });
    }

    if let Some((id, percent)) = changed {
